...
const getSkyMapURL = (starCrossing: Coordinates): string => {
  ...
  const skyMapURL =
    `http://www.wikisky.org/?ra=${urlFriendlyAscension}&de=${urlFriendlyDeclination}=3&show_grid=1&show_constellation_lines=1&show_constellation_boundaries=1&show_const_names=1&show_galaxies=1&img_source=IMG_all}`;
  return skyMapURL;
};
